...
resource "google_container_cluster" "kubernetes" {
  ...
  master_authorized_networks_config  {
    cidr_blocks {
     //cidr_block = "${var.vpc_subnetwork_cidr}"
...
#Get the kubeconfig file from the GKE cluster
resource "null_resource" "configure_kubectl" {
  provisioner "local-exec" {
    command = "gcloud beta container clusters get-credentials ${var.name} --region ${var.region} --project ${var.project}"

    #We can set custom kubecofig file setting enviroment variable KUBECONFIG
    environment = {
